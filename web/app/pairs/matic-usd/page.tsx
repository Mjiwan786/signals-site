/**
 * MATIC/USD Trading Pair Page
 * Live signals filtered for MATIC/USD only
 */
...
function MATICUSDPageContent() {
  const mode = (DEFAULT_MODE as 'paper' | 'live') || 'paper';
  const pair = 'MATIC/USD';
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-12">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-500/10 border border-indigo-500/30 rounded-full mb-4">
            ...
            <span className="text-sm font-medium text-indigo-400">Polygon • Live Data</span>
          </div>

          <h1 className="text-4xl md:text-5xl font-display font-bold text-text mb-3">
            MATIC/USD Trading Signals
          </h1>
          <p className="text-lg text-text2 max-w-2xl">
            AI-powered trading signals exclusively for Polygon. Real-time data streamed via secure Redis connection.
          </p>
        </motion.div>
        ...
        >
          <motion.section variants={fadeInUp} className="scroll-mt-24">
            <div className="mb-6">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h2 className="text-2xl font-bold text-text">MATIC/USD Performance</h2>
              </div>
              <p className="text-dim">
                Track Polygon trading performance across all timeframes
              </p>
            </div>
          ...
          <motion.section variants={fadeInUp}>
            <div className="glass-card rounded-2xl p-8 border-indigo-500/20">
              <h3 className="text-xl font-bold text-text mb-6 flex items-center gap-2">
                <Zap className="w-5 h-5 text-indigo-400" />
                About MATIC/USD Signals
              </h3>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <h4 className="font-semibold text-text2 mb-4">Why Polygon?</h4>
                  <p className="text-sm text-dim mb-4">
                    Polygon (MATIC) is a leading Ethereum scaling solution with a robust ecosystem. Our AI models capture its correlation with Ethereum and unique market movements.
                  </p>
                  <ul className="space-y-2 text-sm text-dim">
                    <li className="flex items-center gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-indigo-400" />
                      Ethereum layer-2 scaling leader
                    </li>
                    <li className="flex items-center gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-indigo-400" />
...
export default function MATICUSDPage() {
  return (
    <PageErrorBoundary pageName="MATIC/USD Signals" fallbackType="signals">
      <MATICUSDPageContent />
    </PageErrorBoundary>
  );
}
